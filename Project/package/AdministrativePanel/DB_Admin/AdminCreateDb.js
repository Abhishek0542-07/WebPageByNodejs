...
var CREATE_TABLE = "CREATE TABLE ADMIN_SECURITY(PRIMARY_KEY INT(6),Designation VARCHAR(225),Email_Id VARCHAR(225),Password VARCHAR(225),Confirm_Password VARCHAR(225),insertdatetime VARCHAR(225))";
checkTableExit(con, 'ADMIN_SECURITY');
    ...
    connection.query('SHOW tables', function(err, tables) {
        ...
        if (STORE_ALL_TABLE_NAME.includes(TableName.toLowerCase())) {
            ...
            con.query(CREATE_TABLE, function(err, result) {
                if (err) throw err;
                con.query(" ALTER TABLE ADMIN_SECURITY ADD COLUMN UpdateTimeDate VARCHAR(225)", function(err, data, fields) {});
...
router.post('/SEND_DATA_LOGIN', async function(req, res) {
    var sql = "Select PRIMARY_KEY from ADMIN_SECURITY ORDER BY PRIMARY_KEY DESC LIMIT 1";
...
function getId(req, res, conn, id) {
    id = id + 1;
    var insert_query = "INSERT INTO ADMIN_SECURITY values('" + id + "','" + req.body.companyname + "','" +
        req.body.email + "','" + req.body.password + "','" + req.body.repeatpassword + "','" +
        req.body.insertdatetime + "','')";
    var sql = "Select Email_Id from ADMIN_SECURITY where Email_Id='" + req.body.email + "'";
    con.query(sql, function(err, data, fields) {
        if (err) throw err;
        if (data == '') {
...
router.post('/ADMIN_LOGIN_DATA', async function(req, res) {
    var sql = "Select Email_Id from ADMIN_SECURITY where Email_Id='" + req.body.email + "' and Password='" + req.body.password + "'";
    con.query(sql, function(err, data, fields) {
        if (err) throw err;
        if (data != '') {
...
router.get('/AdminlogoutPage', async function(req, res) {
